...
func clearCache(cacheType string) error {
	db, err := initDB(); if err != nil { return err }; defer db.Close()
	switch cacheType {
	case "stars": _, err = db.Exec("DELETE FROM repos WHERE type = 'stars'")
	case "repos": _, err = db.Exec("DELETE FROM repos WHERE type = 'repos'")
	case "gists": _, err = db.Exec("DELETE FROM gists")
	case "all":
		if _, err = db.Exec("DELETE FROM repos"); err == nil {
			_, err = db.Exec("DELETE FROM gists")
		}
	default: return fmt.Errorf("unknown cache type: %s", cacheType)
	}
	return err
}

func getCacheInfo(cacheType string) string {
	db, err := initDB(); if err != nil { return fmt.Sprintf("DB Error: %s", err) }; defer db.Close()
	var query string
	switch cacheType {
	case "stars", "repos": query = fmt.Sprintf("SELECT COUNT(*), MAX(fetched_at) FROM repos WHERE type = '%s'", cacheType)
	case "gists": query = "SELECT COUNT(*), MAX(fetched_at) FROM gists"
	default: return "未知类型"
	}
	var count int
	var fetchedAt sql.NullInt64
	if err := db.QueryRow(query).Scan(&count, &fetchedAt); err != nil || count == 0 { return "无缓存" }
	updateTime := "未知"; if fetchedAt.Valid { updateTime = time.Unix(fetchedAt.Int64, 0).Format("2006-01-02 15:04") }
...
func saveRepos(repos []*github.Repository, repoType string) error {
	...
	for _, r := range repos {
		...
		if _, err := stmt.Exec(r.GetID(), repoType, r.GetFullName(), normalize(r.GetFullName()), desc, normalize(desc), r.GetHTMLURL(), r.GetCloneURL(), r.GetStargazersCount(), r.GetPushedAt().Format(time.RFC3339), r.GetPrivate(), r.GetFork(), r.GetArchived(), now); err != nil {
...
func queryRepos(repoType, searchQuery string, limit int) ([]*github.Repository, error) {
	db, err := initDB(); if err != nil { return nil, err }; defer db.Close()
	...
	rows, err := db.Query(query, repoType, normQuery, normQuery, limit); if err != nil { return nil, err }; defer rows.Close()
	var repos []*github.Repository
	for rows.Next() {
		var r github.Repository
		var id sql.NullInt64; var fullName, desc, htmlURL, cloneURL, updatedAtStr sql.NullString
		var stars sql.NullInt64; var private, fork, archived sql.NullBool
		if err := rows.Scan(&id, &fullName, &desc, &htmlURL, &cloneURL, &stars, &updatedAtStr, &private, &fork, &archived); err != nil { return nil, err }
		...
		if t, err := time.Parse(time.RFC3339, updatedAtStr.String); err == nil { r.PushedAt = &github.Timestamp{Time: t} }
...
func saveGists(gists []*github.Gist) error {
	...
	now := time.Now().Unix()
	for _, g := range gists {
		var filenames []string; var normFilenames []string
		for f := range g.Files { filenames = append(filenames, string(f)); normFilenames = append(normFilenames, normalize(string(f))) }
...
func queryGists(searchQuery string, limit int) ([]*github.Gist, error) {
	...
	query := `SELECT id, description, html_url, public, updated_at, files FROM gists
              ...
              ORDER BY updated_at DESC LIMIT ?`
	rows, err := db.Query(query, normQuery, normQuery, limit); if err != nil { return nil, err }; defer rows.Close()
	var gists []*github.Gist
	for rows.Next() {
		var g github.Gist
		var id, desc, htmlURL, updatedAtStr, filesJSON sql.NullString
		var public sql.NullBool
		if err := rows.Scan(&id, &desc, &htmlURL, &public, &updatedAtStr, &filesJSON); err != nil { return nil, err }
		...
		if t, err := time.Parse(time.RFC3339, updatedAtStr.String); err == nil { g.UpdatedAt = &github.Timestamp{Time: t} }
		var filenames []string
		if err := json.Unmarshal([]byte(filesJSON.String), &filenames); err == nil {
			g.Files = make(map[github.GistFilename]github.GistFile)
			for _, f := range filenames { g.Files[github.GistFilename(f)] = github.GistFile{} }
		}
